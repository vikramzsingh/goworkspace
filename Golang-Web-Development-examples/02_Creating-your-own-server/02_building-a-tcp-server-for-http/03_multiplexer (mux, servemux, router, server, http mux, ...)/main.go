...
func handel(conn net.Conn) {
	defer conn.Close() // execute at the end of this code body
	//read request
	request(conn)
}
...
func request(conn net.Conn) {
	...
	for scanner.Scan() {
		ln := scanner.Text()
		fmt.Println(ln) // prints out the first line readed i.e GET/ http/1.1 -->[]string
		if i == 0 {
			mux(conn, ln) // passing connection and first line to, deal with METHOD and URL.
		}
		if ln == "" { // for breaking the loof at the end
			// header are done
			break
		}
		i++
...
func mux(conn net.Conn, ln string) {
	// separate METHOD and URL from comming line. --> []string
	// request line
